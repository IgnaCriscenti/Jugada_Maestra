...
namespace TableroApuestas.Data
{
    ...
    ";
        ...
        #region EQUIPOS - JUGADORES - FIXTURE
        ...
        public async Task<DataTable> ObtenerUsuariosConTotalApostadoAsync()
        {
            string sql = @"
        SELECT 
            u.id_usuario,
            u.nombre,
            u.apellido,
            IIf(SUM(a.monto) Is Null, 0, SUM(a.monto)) AS TotalApostado,
            IIf(SUM(IIf(a.estado='Ganada', a.monto, 0)) Is Null, 0, SUM(IIf(a.estado='Ganada', a.monto, 0))) AS MontoGanado,
            IIf(SUM(IIf(a.estado='Perdida', a.monto, 0)) Is Null, 0, SUM(IIf(a.estado='Perdida', a.monto, 0))) AS MontoPerdido
        FROM usuarios AS u
        LEFT JOIN apuesta AS a ON u.id_usuario = a.id_usuario
        GROUP BY u.id_usuario, u.nombre, u.apellido
        ORDER BY SUM(a.monto) DESC";  // ranking de mayor a menor
